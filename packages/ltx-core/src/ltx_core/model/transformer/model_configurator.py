...
class LTXModelConfigurator(ModelConfigurator[LTXModel]):
    ...
    def from_config(cls: type[LTXModel], config: dict) -> LTXModel:
        ...
        ca_adaln = config.get("cross_attention_adaln", config.get("av_cross_ada_norm", True))
        print(f"DEBUG: cross_attention_adaln derived from config: {ca_adaln}")
        ...
        return LTXModel(
            ...
            cross_attention_adaln=config.get("cross_attention_adaln", config.get("av_cross_ada_norm", True)),
        )
...
class LTXVideoOnlyModelConfigurator(ModelConfigurator[LTXModel]):
    ...
    def from_config(cls: type[LTXModel], config: dict) -> LTXModel:
        ...
        return LTXModel(
            ...
            apply_gated_attention=config.get("apply_gated_attention", False),
            caption_projection=caption_projection,
            cross_attention_adaln=config.get("cross_attention_adaln", config.get("cross_attention_norm", True)),
        )
