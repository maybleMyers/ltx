...
# Use ltx2.3 packages instead of the modified packages/
# This ensures the model architecture matches the unmodified 2.3 checkpoint exactly.
_script_dir = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, os.path.join(_script_dir, "ltx2.3", "packages", "ltx-pipelines", "src"))
sys.path.insert(0, os.path.join(_script_dir, "ltx2.3", "packages", "ltx-core", "src"))
...
def _import_from_modified_packages(module_name, file_path):
    """Import a module from the modified packages/ directory by file path."""
...
_modified_pkg = os.path.join(_script_dir, "packages", "ltx-pipelines", "src", "ltx_pipelines", "utils")
_import_from_modified_packages(
    "ltx_pipelines.utils.custom_offloading_utils_simple",
    os.path.join(_modified_pkg, "custom_offloading_utils_simple.py"),
)
_custom_offloading = _import_from_modified_packages(
    "ltx_pipelines.utils.custom_offloading_utils",
    os.path.join(_modified_pkg, "custom_offloading_utils.py"),
)
clean_memory_on_device = _custom_offloading.clean_memory_on_device

_import_from_modified_packages(
    "ltx_pipelines.utils.block_swap_simple",
    os.path.join(_modified_pkg, "block_swap_simple.py"),
)
_block_swap = _import_from_modified_packages(
    "ltx_pipelines.utils.block_swap",
    os.path.join(_modified_pkg, "block_swap.py"),
)
enable_block_swap = _block_swap.enable_block_swap
enable_block_swap_with_activation_offload = _block_swap.enable_block_swap_with_activation_offload
offload_all_blocks = _block_swap.offload_all_blocks
enable_text_encoder_block_swap = _block_swap.enable_text_encoder_block_swap
offload_all_text_encoder_blocks = _block_swap.offload_all_text_encoder_blocks
...
# Constants not in the unmodified 2.3 constants module
...
from ltx_pipelines.utils.samplers import euler_denoising_loop, gradient_estimating_euler_denoising_loop, res2s_audio_video_denoising_loop
...
# =============================================================================
# Compatibility shim for encode_text (not in unmodified 2.3 ltx_core)
# =============================================================================

class CombinedTextEncoder(torch.nn.Module):
    """Wraps a 2.3 GemmaTextEncoder + EmbeddingsProcessor into a single callable.

    The unmodified 2.3 architecture separates text encoding (GemmaTextEncoder)
    from embedding projection (EmbeddingsProcessor). This wrapper combines them
    so calling combined_encoder(prompt) returns (v_context, a_context, mask)
    matching the modified packages' API.
    """
    ...
    # Delegate enhance/generate methods to the underlying text encoder
    def _enhance(self, *args, **kwargs):
    ...
    @property
    def processor(self):
        return self.text_encoder.processor

    @processor.setter
    def processor(self, value):
        self.text_encoder.processor = value

    def _init_image_processor(self):
        if hasattr(self.text_encoder, '_init_image_processor'):
            self.text_encoder._init_image_processor()

    def enhance(self, *args, **kwargs):
        if hasattr(self.text_encoder, 'enhance'):
...
def build_combined_text_encoder(model_ledger, device_override=None):
    """Build a CombinedTextEncoder from a ModelLedger (2.3 architecture).

    In 2.3, text encoding is split into GemmaTextEncoder + EmbeddingsProcessor.
    This builds both and wraps them so the rest of the script can use the same API.
    """
    original_device = None
    if device_override is not None:
        original_device = model_ledger.device
        model_ledger.device = device_override

    text_encoder = model_ledger.text_encoder()
    embeddings_processor = model_ledger.gemma_embeddings_processor()

    if original_device is not None:
        model_ledger.device = original_device
...
# =============================================================================
# Latent normalization functions (from modified packages, not in unmodified 2.3)
# =============================================================================

def adain_normalize(latent, reference, factor=1.0, per_frame=False):
    """Adaptive Instance Normalization for latents."""
    if factor == 0.0:
        return latent
    t = latent.clone()
    ndim = t.ndim
        ...
        for i in range(t.size(0)):
            for c in range(t.size(1)):
                if not per_frame:
                    ...
                    ref = reference
                    if ref.size(2) == 1:
                        ref = ref.repeat(1, 1, t.size(2), 1, 1)
                    for f in range(t.size(2)):
                        r_sd, r_mean = torch.std_mean(ref[i, c, f], dim=None)
...
def statistical_normalize(latent, target_mean=0.0, target_std=1.0, percentile=95.0, factor=1.0, clip_outliers=False):
    ...
    def _normalize_channel(channel_data):
        ...
        flat_f32 = flat.float()
        lb = torch.quantile(flat_f32, lower_pct / 100).to(flat.dtype)
        ub = torch.quantile(flat_f32, upper_pct / 100).to(flat.dtype)
        mask = (flat >= lb) & (flat <= ub)
        if mask.sum() > 0:
            filtered = flat[mask]
            cur_mean, cur_std = filtered.mean(), filtered.std()
            if cur_std > 1e-8:
                normed = ((flat - cur_mean) / cur_std) * target_std + target_mean
                if clip_outliers:
                    n_lb = ((lb - cur_mean) / cur_std) * target_std + target_mean
                    n_ub = ((ub - cur_mean) / cur_std) * target_std + target_mean
                    normed = torch.clamp(normed, n_lb, n_ub)
                return normed.reshape(channel_data.shape)
            else:
                return channel_data - cur_mean + target_mean
        return channel_data

    if t.ndim == 5:
        for i in range(t.size(0)):
            for c in range(t.size(1)):
                t[i, c] = _normalize_channel(t[i, c])
    elif t.ndim == 3:
        for i in range(t.size(0)):
            for c in range(t.size(1)):
                t[i, c] = _normalize_channel(t[i, c])
...
def create_per_step_stat_norm_fn(factors, target_mean=0.0, target_std=1.0, percentile=95.0, clip_outliers=False, apply_to_video=True, apply_to_audio=True):
    """Create a per-step statistical normalization function for denoising loops."""
    factor_list = [float(x.strip()) for x in factors.split(",")] if isinstance(factors, str) else list(factors)
    def norm_fn(video_latent, audio_latent, step_idx):
        factor = factor_list[min(step_idx, len(factor_list) - 1)]
        if factor == 0.0:
            return video_latent, audio_latent
        if apply_to_video:
            video_latent = statistical_normalize(video_latent, target_mean=target_mean, target_std=target_std, percentile=percentile, factor=factor, clip_outliers=clip_outliers)
        if apply_to_audio:
            audio_latent = statistical_normalize(audio_latent, target_mean=target_mean, target_std=target_std, percentile=percentile, factor=factor, clip_outliers=clip_outliers)
        return video_latent, audio_latent
    return norm_fn


def create_per_step_adain_norm_fn(factors, reference_video=None, reference_audio=None, per_frame=False):
    """Create a per-step AdaIN normalization function for denoising loops."""
    factor_list = [float(x.strip()) for x in factors.split(",")] if isinstance(factors, str) else list(factors)
    def norm_fn(video_latent, audio_latent, step_idx):
        factor = factor_list[min(step_idx, len(factor_list) - 1)]
        if factor == 0.0:
        ...
        if reference_video is not None:
            video_latent = adain_normalize(video_latent, reference_video, factor=factor, per_frame=per_frame)
        if reference_audio is not None:
            audio_latent = adain_normalize(audio_latent, reference_audio, factor=factor, per_frame=False)
        return video_latent, audio_latent
    return norm_fn
