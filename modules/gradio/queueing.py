        ...
        try:
            ...
            if response and response.get("is_generating", False):
                ...
                while response and response.get("is_generating", False):
                    ...
                    # Don't stop processing when clients disconnect - just stop sending messages
                    # This allows long-running jobs to complete even if browser closes
                    awake_events = [event for event in awake_events if event.alive]
                    # Continue processing with original events even if all clients disconnected
                    if not awake_events:
                        awake_events = events  # Keep processing with original events
                    try:
                        start = time.monotonic()
                        ...
                        # Don't break when clients disconnect - continue processing
                        if not awake_events:
                            awake_events = events  # Keep processing with original events
